...
formatter = logging.Formatter(
    "%(levelname)s line %(lineno)d:  %(asctime)s:  %(stack_info)s:  %(name)s:%(message)s"
)
...
class Controller:
    def __init__(self, model: Model, view: View):
        ...
        self.button_color_scheme = ButtonColor(
            primary="#2ebed1", warning="#dbc925", danger="red", success="green"
        )
        ...
        self.style.configure(
            "Success.TButton",
            background=self.button_color_scheme["Success"],
    ...
    def search_for_employee_click_event_handler(self) -> None:
        ...
        if employee_check in hub.employee_list[0].name:
            ...
            logger.debug(
                f"Employee: {employee_check} not found in employee list: {hub.employee_list}"
            )
            view.current_employee_selected.insert(
                tk.END, f"Employee {employee_check} not found"
            )
    ...
    def add_new_employee_click_event_handler(self) -> None:
        ...
        if not self._found_employee:
            ...
            logger.info(
                "Button to add employee is disabled because employee was not found."
            )
            ...
            logger.info(
                f"Employee: {view.current_employee_selected.get(tk.END)} added to employee list"
            )
    ...
    def add_new_hourly_wage_click_event_handler(self) -> None:
        ...
        if self._found_employee:
            ...
            employee.hourly_wage = int(
                view.add_new_hourly_wage_for_employee_entry.get()
            )
            logger.info(
                f"Hourly wage: {employee.hourly_wage} added to employee: {employee.name}"
            )
            view.add_new_hourly_wage_for_employee_button.configure(
    ...
    def employee_payout(self) -> str|None:
        ...
        if self._found_employee:
            try:
                ...
                logger.warning(
                    f"{employee.name} does not have their hours in the system yet or is not eligible for pay \n employee might not have an employee ID."
                )
                logger.debug(
                    f"{employee.name} eligible to work: {employee.is_eligible_to_work()}"
                )
...
class View(tk.Frame):
    def __init__(self, parent):
        ...
        self.style.configure(
            "My.TButton",
            foreground="black",
            highlightbackground=self.button_color_scheme["Primary"],
            background=self.button_color_scheme["Primary"],
        )

        self.employee_lookup_button = ttk.Button(
            self, command=self.employee_lookup, text="Search", style="My.TButton"
        )
        ...
        #
        self.add_new_employee_button = ttk.Button(
            self, command=self.add_new_employee, text="Add Employee", style="My.TButton"
        )
        ...
        self.add_new_hourly_wage_for_employee_button = ttk.Button(
            self,
            text="Submit",
            command=self.add_new_hourly_wage_for_employee,
            style="My.TButton",
        )
        ...
        self.add_weekly_hours_for_employee_button = ttk.Button(
            self,
            command=self.add_weekly_hours_for_employee,
            text="Submit",
            style="My.TButton",
        )
        ...
        self.calculate_weekly_pay_button = ttk.Button(
            self, command=self.calculate_weekly_pay, text="Submit", style="My.TButton"
        )
        ...
        self.submit_employee_info_form_button = ttk.Button(
            text="Submit Employee Info",
            command=self.submit_employee_info,
            style="My.TButton",
        )
